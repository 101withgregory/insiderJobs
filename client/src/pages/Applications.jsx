        ...
        <div className='flex gap-2 mb-6 mt-3'>
            {
                ...
                <label className='flex items-center' htmlFor="resumeUpload">
                    ...
                    <input onChange={e=>setResume(e.target.files[0])} type="file" accept='application/pdf' id='resumeUpload' hidden/>
                    <img src={assets.profile_upload_icon} alt="" className='cursor-pointer'/>
                </label>
                <button className='bg-green-100 border border-green-400 px-4 py-2 rounded-lg cursor-pointer' onClick={updateResume}>save</button>
                </>:
                <div className='flex gap-2'>
                  <a target='_blank' href={userData.resume} className='bg-blue-100 text-blue-600 px-4 py-2 rounded-lg cursor-pointer'>
                    Resume
                  </a>
          ...
          <tbody>
            {
              userApplications.map((job,index)=> true ? (
                <tr key={index}>
                  <td className='py-3 px-4 flex items-center gap-2 border-b'><img src={job.companyId.image} alt="" className='w-8 h-8'/>
                  {job.companyId.name}</td>
